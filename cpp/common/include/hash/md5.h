 ...
 */
inline void md5Step(uint32_t* buffer, uint32_t* input)
{
 ...
 */
inline void md5Update(MD5Context* ctx, uint8_t* input_buffer, size_t input_len)
{
    uint32_t input[16];
 ...
 */
inline std::array<uint8_t, 16> md5String(std::string_view message)
{
    MD5Context ctx;
    md5Init(&ctx);
    md5Update(&ctx, (uint8_t*)message.data(), message.length());
    md5Finalize(&ctx);
